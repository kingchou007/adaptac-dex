...
from adaptac.model.minkowski.resnet import ResNet14, ResNet18


class Sparse3DEncoder(torch.nn.Module):
    def __init__(self, input_dim=6, output_dim=512, backbone="resnet14"):
        super().__init__()
    ...
    def forward(self, sinput, max_num_token=100, batch_size=24, mask_ratio=0):
        """
        Forward pass for sparse 3D encoder.

        Args:
            sinput: Sparse input tensor
            max_num_token: Maximum token number for each point cloud (100 for voxel_size=0.005)
            batch_size: Batch size
            mask_ratio: Ratio of tokens to mask (0 means no masking, deprecated - kept for compatibility)
        """
        ...
        pos_list = self.position_embedding(coords_list)  # show number of tokens
        tokens = torch.zeros(
            [batch_size, max_num_token, self.output_dim],
            dtype=feats_batch.dtype,
        ...
        token_padding_mask = torch.ones(
            [batch_size, max_num_token], dtype=torch.bool, device=feats_batch.device
        )
        ...
        # Note: mask_ratio parameter is deprecated and ignored
        # Masking functionality has been removed as it's not used in the current implementation
...
class SparsePositionalEncoding(nn.Module):
    ...
    def _init_position_vector(self):
        ...
        self.pos_x_vector = torch.stack(
            [pos_x_vector[:, 0::2].sin(), pos_x_vector[:, 1::2].cos()], dim=2
        ).flatten(1)
        self.pos_y_vector = torch.stack(
            [pos_y_vector[:, 0::2].sin(), pos_y_vector[:, 1::2].cos()], dim=2
        ).flatten(1)
        self.pos_z_vector = torch.stack(
            [pos_z_vector[:, 0::2].sin(), pos_z_vector[:, 1::2].cos()], dim=2
        ).flatten(1)
