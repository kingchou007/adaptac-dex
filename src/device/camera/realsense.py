...
class RealSenseRGBDCamera:
    ...
    def __init__(
        self, serial, frame_rate=30, resolution=(1280, 720), align=True, **kwargs
    ):
        """
        Initialization.
        ...
        self.config.enable_stream(
            rs.stream.color, resolution[0], resolution[1], rs.format.bgr8, frame_rate
        )
        try:
    ...
    def get_rgbd_image(self):
        ...
        color_image = np.asanyarray(frameset.get_color_frame().get_data()).astype(
            np.uint8
        )
        depth_image = (
            np.asanyarray(frameset.get_depth_frame().get_data()).astype(np.float32)
            / self.depth_scale
        )
